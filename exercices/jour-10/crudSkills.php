...
class ProductRepository
{
    ...
    public function find($id)
    {
        ...
        if ($row) {
            return new Product(             // HYDRATATION : On transforme le tableau $row en objet Product
                $row['id'], 
                $row['name'], 
                $row['description'], 
                $row['price'], 
    ...
    public function save($product)     // SAVE (Mettre à jour) si l'ID est vide, c'est une création
    {
        if ($product->id === null) {
            ...
            $product->id = $this->pdo->lastInsertId();     // On récupère l'ID créé et on le met dans l'objet
        } 
